                ...
                BootstrapDialog.show({
                    ...
                    message: function (diaSelf) {
                        ...
                        for (let key in stuMirrorSup) {
                            switch (key){
                                case 'sex':
                                    msg += `<label>${stuMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男">男</option><option value="女">女</option></select>`;
                                    break;
                                case 'semester':
                                    msg += `<label>${stuMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春">春</option><option value="夏">夏</option><option value="秋">秋</option><option value="冬">冬</option></select>`;
                                    break;
                                case 'salary':
                                case 'classHour':
                                case 'credit':
                                    msg += `<label>${stuMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" placeholder="Enter ${stuMirrorSup[key]}" min="0" step="1">`;
                                    break;
                                default:
                                ...
                                message: function (diaSelf) {
                                    ...
                                    for (let key in studentInfo) {
                                        if (stuMirrorSup.hasOwnProperty(key)) {
                                            ...
                                            switch (key){
                                                case 'sex':
                                                    msg += `<label>${stuMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男" ${val==='男'?'selected':''}>男</option><option value="女" ${val==='女'?'selected':''}>女</option></select>`;
                                                    break;
                                                case 'semester':
                                                    msg += `<label>${stuMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春" ${val==='春'?'selected':''}>春</option><option value="夏" ${val==='夏'?'selected':''}>夏</option><option value="秋" ${val==='秋'?'selected':''}>秋</option><option value="冬" ${val==='冬'?'selected':''}>冬</option></select>`;
                                                    break;
                                                case 'salary':
                                                case 'classHour':
                                                case 'credit':
                                                    msg += `<label>${stuMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" value="${val}" min="0" step="1">`;
                                                    break;
                                                default:
                ...
                BootstrapDialog.show({
                    ...
                    message: function (diaSelf) {
                        ...
                        for (let key in teaMirrorSup) {
                            switch (key){
                                case 'sex':
                                    msg += `<label>${teaMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男">男</option><option value="女">女</option></select>`;
                                    break;
                                case 'semester':
                                    msg += `<label>${teaMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春">春</option><option value="夏">夏</option><option value="秋">秋</option><option value="冬">冬</option></select>`;
                                    break;
                                case 'salary':
                                case 'classHour':
                                case 'credit':
                                    msg += `<label>${teaMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" placeholder="Enter ${teaMirrorSup[key]}" min="0" step="1">`;
                                    break;
                                default:
                                ...
                                message: function (diaSelf) {
                                    ...
                                    for (let key in teacherInfo) {
                                        if (teaMirrorSup.hasOwnProperty(key)) {
                                            ...
                                            switch (key){
                                                case 'sex':
                                                    msg += `<label>${teaMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男" ${val==='男'?'selected':''}>男</option><option value="女" ${val==='女'?'selected':''}>女</option></select>`;
                                                    break;
                                                case 'semester':
                                                    msg += `<label>${teaMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春" ${val==='春'?'selected':''}>春</option><option value="夏" ${val==='夏'?'selected':''}>夏</option><option value="秋" ${val==='秋'?'selected':''}>秋</option><option value="冬" ${val==='冬'?'selected':''}>冬</option></select>`;
                                                    break;
                                                case 'salary':
                                                case 'classHour':
                                                case 'credit':
                                                    msg += `<label>${teaMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" value="${val}" min="0" step="1">`;
                                                    break;
                                                default:
                ...
                BootstrapDialog.show({
                    ...
                    message: function (diaSelf) {
                        ...
                        for (let key in crsMirrorSup) {
                            switch (key){
                                case 'sex':
                                    msg += `<label>${crsMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男">男</option><option value="女">女</option></select>`;
                                    break;
                                case 'semester':
                                    msg += `<label>${crsMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春">春</option><option value="夏">夏</option><option value="秋">秋</option><option value="冬">冬</option></select>`;
                                    break;
                                case 'salary':
                                case 'classHour':
                                case 'credit':
                                    msg += `<label>${crsMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" placeholder="Enter ${crsMirrorSup[key]}" min="0" step="1">`;
                                    break;
                                default:
                                ...
                                message: function (diaSelf) {
                                    ...
                                    for (let key in courseInfo) {
                                        if (crsMirrorSup.hasOwnProperty(key)) {
                                            ...
                                            switch (key){
                                                case 'sex':
                                                    msg += `<label>${crsMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="男" ${val==='男'?'selected':''}>男</option><option value="女" ${val==='女'?'selected':''}>女</option></select>`;
                                                    break;
                                                case 'semester':
                                                    msg += `<label>${crsMirrorSup[key]}</label><select name="${key}" class="form-control"><option value="春" ${val==='春'?'selected':''}>春</option><option value="夏" ${val==='夏'?'selected':''}>夏</option><option value="秋" ${val==='秋'?'selected':''}>秋</option><option value="冬" ${val==='冬'?'selected':''}>冬</option></select>`;
                                                    break;
                                                case 'salary':
                                                case 'classHour':
                                                case 'credit':
                                                    msg += `<label>${crsMirrorSup[key]}</label><input name="${key}" type="number" class="form-control" value="${val}" min="0" step="1">`;
                                                    break;
                                                default:
